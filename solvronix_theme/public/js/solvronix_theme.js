...
(function () {
  ...
  function buildBreadcrumb() {
    ...
    $("#st-breadcrumb").remove();

    var route = frappe.get_route ? frappe.get_route() : [];
    if (!route || route.length < 2) return;

    var parts = [];
    parts.push('<a href="/desk/home">Home</a>');
    ...
    if (r0 === "form") {
      /* route = ["form", "DocType", "DocName"]
         "Form" itself is not a valid page — link to the list view instead,
         or skip it entirely for Single doctypes. */
      var doctype = route[1] || "";
      var docname = route[2] || "";
      var isSingle = doctype === docname;
      var dtLabel = titleCase(doctype);

      if (isSingle) {
        /* Single doctype — just show its name as the current page */
        parts.push('<span class="st-sep">›</span><span>' + dtLabel + '</span>');
      } else {
        /* Regular form — link to list view, then doc name */
        var slug = frappe.router ? frappe.router.slug(doctype) : doctype.toLowerCase().replace(/\s+/g, "-");
        parts.push('<span class="st-sep">›</span><a href="/desk/' + slug + '">' + dtLabel + '</a>');
        if (docname && docname !== doctype) {
          parts.push('<span class="st-sep">›</span><span>' + docname + '</span>');
        }
      }
    } else if (r0 === "list") {
      /* route = ["list", "DocType", ...] — list view, link is already current page */
      parts.push('<span class="st-sep">›</span><span>' + titleCase(route[1]) + '</span>');
    } else if (r0 !== "home" && r0 !== "smart-home" && r0 !== "workspaces") {
      /* Workspace / custom page — use route[0] as the module link */
      parts.push('<span class="st-sep">›</span><a href="/desk/' + route[0] + '">' + titleCase(route[0]) + '</a>');
      if (route.length >= 2 && route[1] && route[1] !== route[0]) {
        parts.push('<span class="st-sep">›</span><span>' + titleCase(route[1]) + '</span>');
      }
    }
    ...
    var $bc = $('<div id="st-breadcrumb">' + parts.join("") + "</div>");

    var $titleEl = $pageHead.find(".page-title, .title-text").first();
    if ($titleEl.length) {
      $titleEl.before($bc);
    } else {
      $pageHead.prepend($bc);
  ...
  function injectSetupGuide() {
    /* Only System Manager, only once */
    if (!frappe.user_roles || !frappe.user_roles.includes("System Manager")) return;
    if (localStorage.getItem("st_setup_v1") === "done") return;
    ...
    if (step1done && step2done && step3done) {
      localStorage.setItem("st_setup_v1", "done");
      return;
    }
  ...
  function injectTopToolbar() {
    ...
    function openLangDrop() {
      ...
      if (!_allLangs.length) {
        frappe.call({
          method: "solvronix_theme.api.get_available_languages",
          callback: function (r) {
            _allLangs = r.message || [];
    ...
    $searchBar.on("click", function () {
      var cp = window.solvronix_theme && window.solvronix_theme.cp;
    ...
    var $userBtn = $(
      '<button id="st-user-btn" title="Account">' +
        '<span class="st-user-av">' + initials + '</span>' +
    ...
    var $userDrop = $(
      '<div id="st-user-dropdown">' +
        '<div class="st-ud-header">' +
          '<div class="st-ud-av">' + initials + '</div>' +
    ...
    $userDrop.on("click", "[data-action]", function () {
      ...
      if (action === "edit-profile") {
        ...
        frappe.confirm(frappe._("Reset your desktop layout to default?"), function () {
          frappe.call({
            method: "solvronix_theme.api.reset_workspace_for_user",
  ...
  function switchLanguage(code) {
    frappe.call({
      method: "solvronix_theme.api.set_user_language",
      args: { lang_code: code },
      callback: function () {
